...
//			this.table = table;
	...
	public synchronized void takeOrder(Table table, int orderNum, int customerNum){
		//pass this order to the cook
		Order newOrder = new Order(orderNum, customerNum, table, this);
		CookFactory.addOrder(newOrder);
		
	}
	
	public synchronized void addCompletedOrder(Order completedOrder){
	...
	public void run() {
		try {
			while (true) {
				...
				this.waiterLock.unlock();
				Thread.sleep(1000 * (int)(Math.random() * 2)); // sleep for between 0 and 2 seconds (smoke break)
				if (getTable(0) != null) {
				...
				//TODO Change the Waiter code so that they check, every so often, for completed orders. 
				//Since Waiters can have multiple tables, you need a Vector<Order> 
				//for each Waiter that the Cooks populate when an order is cooked. 
				//If the Vector is not empty, the Waiter is to “deliver” the food to the Customer. 
				//Since Customers are waiting for their food, the Waiter will have to signal the Customer.
				if(completedOrders.size() > 0){
					try{
						...
						//deliver the food to customer signal
						completedOrder.getTable().getCustomer().orderIsNowReady();
						
						//The Waiter is to print a message to the regular window: Waiter X has delivered order Y to table Z.”.
						Restaurant.addWaiterMessage("Waiter " + this.waiterNumber + " has delivered order " + 
								completedOrder.getOrder() + "to table " +
								completedOrder.getTable().getTableNumber(), this.waiterNumber);
					} finally{
						this.waiterLock.unlock();
					}
